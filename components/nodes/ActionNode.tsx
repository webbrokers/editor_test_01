...
export default function ActionNode({ data, id, selected }: NodeProps<ActionNodeData>) {
    ...
    const getActionText = (type: string) => {
        switch (type) {
            case 'add_points': return 'Начислить баллы';
            case 'subtract_points': return 'Списать баллы';
            case 'add_spins': return 'Начислить спины';
            case 'level_up': return 'Повысить уровень';
    ...
    return (
        <div
            className={`glass rounded-xl p-4 min-w-[280px] border-2 transition-all ${selected ? 'border-accent shadow-lg shadow-accent/30' : 'border-white/10'
                }`}
