        ...
        >
            ...
            {data.startDate && (
                <div className="text-xs text-foreground/60 mb-2">
                    <span>Начало: {new Date(data.startDate).toLocaleDateString('ru-RU')}</span>
                </div>
            )}

            {data.status && (
                <div className="mt-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${data.status === 'active' ? 'bg-green-500/20 text-green-400' :
                            data.status === 'paused' ? 'bg-yellow-500/20 text-yellow-400' :
                                'bg-gray-500/20 text-gray-400'
                        }`}>
                        {data.status === 'active' ? 'Активна' :
                            data.status === 'paused' ? 'Приостановлена' : 'Черновик'}
                    </span>
                </div>
            )}
